...
const Artifact3DViewer: React.FC<Artifact3DViewerProps> = ({
    ...
    return (
        <div className="relative w-full h-[300px] rounded-xl overflow-hidden">
            ...
            {loading && (
                <div
                    className={`absolute inset-0 z-20 flex flex-col items-center justify-center bg-[#121520] gap-3 transition-all duration-400 ease-out ${
                        isExitingLoader ? 'opacity-0 scale-105 pointer-events-none' : 'opacity-100 scale-100'
                    }`}
                >
            ...
            <div
                ...
                className={`w-full h-full cursor-grab active:cursor-grabbing transition-opacity duration-500 ${
                    loading && !isExitingLoader ? 'opacity-0' : 'opacity-100'
                }`}
            />
        </div>
    );
...
export const ArtifactDetailModal: React.FC<ArtifactDetailModalProps> = ({
    ...
    return (
        <div
            className={`fixed inset-0 w-screen h-screen bg-black/40 backdrop-blur-xs flex justify-end items-stretch z-100 ${isClosing ? 'animate-fade-out-backdrop' : 'animate-fade-in-backdrop'
                }`}
            onClick={onClose}
        >
            <div
                className={`w-[520px] max-w-[90vw] h-screen bg-[#141824]/95 border-l border-white/15 rounded-l-3xl p-7 shadow-[-12px_0_45px_0_rgba(0,0,0,0.75)] flex flex-col gap-4 relative text-slate-50 overflow-y-auto ${isClosing ? 'animate-slide-out-right' : 'animate-slide-in-right'
                    }`}
                ...
                <div className="bg-[#121520] rounded-[18px] border border-white/10 p-3 flex flex-col items-center gap-2.5 relative min-h-[350px] shrink-0">
                    ...
                    <div className="absolute top-3 right-3 flex items-center gap-1.5 z-15">
                        ...
                        <button
                            ...
                            className={`px-2 h-7 rounded-lg border text-xs font-semibold cursor-pointer flex items-center gap-1 transition-all ${autoRotate
                    ...
                    <div className="w-full flex flex-wrap items-center justify-between gap-2 px-2.5 py-2 bg-slate-900/80 rounded-xl border border-white/10">
                        ...
                        <div className="flex items-center gap-1 bg-black/40 p-0.5 rounded-lg border border-white/10">
                            <button
                                ...
                                className={`px-2.5 py-1 rounded-md text-xs font-medium cursor-pointer transition-all flex items-center gap-1 ${lightMode === 'studio'
                            ...
                            <button
                                ...
                                className={`px-2.5 py-1 rounded-md text-xs font-medium cursor-pointer transition-all flex items-center gap-1 ${lightMode === 'unlit'
